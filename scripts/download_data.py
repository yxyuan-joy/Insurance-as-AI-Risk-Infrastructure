...
DEFAULT_REPO_ID = ""
DEFAULT_BASE_URL = "https://anonymous-hf.com/api/a/v3b4fjkrkwi9/resolve"
...
def download_anonymous_file(base_url: str, relative_path: str, destination: Path) -> None:
    encoded_path = "/".join(urllib.parse.quote(part, safe="") for part in relative_path.split("/"))
    url = f"{base_url.rstrip('/')}/{encoded_path}"
    request = urllib.request.Request(url, headers={"User-Agent": "anonymous-review-artifact/1.0"})
    destination.parent.mkdir(parents=True, exist_ok=True)
    temporary = destination.with_suffix(destination.suffix + ".part")
    try:
        context = ssl.create_default_context(cafile=certifi.where())
        with urllib.request.urlopen(request, timeout=300, context=context) as response, temporary.open("wb") as handle:
            shutil.copyfileobj(response, handle)
        temporary.replace(destination)
    except urllib.error.HTTPError as exc:
        temporary.unlink(missing_ok=True)
        raise RuntimeError(f"Could not download {relative_path}: HTTP {exc.code} from the anonymous dataset") from exc
    except Exception:
        temporary.unlink(missing_ok=True)
        raise
...
def main() -> None:
    ...
    parser.add_argument("--base-url", default=os.environ.get("HF_DATASET_BASE_URL", DEFAULT_BASE_URL))
    ...
    if args.base_url:
        for relative_path in REQUIRED_FILES:
            ...
            download_anonymous_file(args.base_url, relative_path, destination)
    elif args.repo_id:
        ...
        raise RuntimeError("Set HF_DATASET_BASE_URL or HF_DATASET_REPO before downloading data.")
